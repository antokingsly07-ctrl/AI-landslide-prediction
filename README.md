...
- **Live AI prediction UI** — `/predict` page with click-to-pick map coordinates, editable environmental features, live weather auto-fill, and per-factor contribution breakdown.
- **7-day risk & rainfall forecast** — lightweight statistical time-series forecasting endpoint (`/ml/forecast`) driving an outlook panel.
...
- **Alerts & notifications** — multilingual verified templates (EN/HI/AS/BN/TA); **real-time WebSocket push** (toast alerts on `/ws/alerts`) plus SMS/Email/Push/InApp providers (mocked in demo).
- **Background risk monitor** — autonomous alert loop (`app.tasks.monitor`) started on FastAPI startup.
- **Sensors & weather** — sensor readings with anomaly detection; pluggable IMD/weather and satellite providers (mocked in demo).
- **Role-based access** — Super Admin, District Admin, Disaster Mgmt, Field Officer, Citizen. Self-registration hardened to non-privileged roles; login/register rate-limited.
- **Persistent admin thresholds** — risk level ranges stored in DB (`system_config`) and applied immediately, no restart.
- **CSV export** — field reports and risk predictions downloadable as CSV.
- **Emergency priorities** — automated incident prioritization for response teams.

## Tech Stack

- **Backend**: Python · FastAPI · SQLAlchemy · Pydantic v2 · JWT · Alembic
...
- **Realtime**: FastAPI WebSocket + in-app toast notifications
...
# Persists PostGIS volume for data; includes auto-seed on first boot.
```
...
The repo ships `alembic.ini` + `database/migrations/` (env.py + a starter migration for
`system_config`). To generate the full authoritative schema migration after changing models:

```bash
cd backend && python -m pip install alembic   # if not installed
cd .. && python scripts/make_migration.py -m "initial schema"
