...
def update_road_status(id: str, payload: RoadStatusUpdate,
    ...
    if payload.status in ("blocked", "severely_blocked"):
        status_label = ROAD_STATUS_LABELS.get(payload.status, {}).get("en", payload.status)
        title = translate("road_title", "en", road=road.name)
        msg = translate("road_message", "en", road=road.name, status=status_label)
        create_alert(
