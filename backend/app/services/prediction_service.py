...
def _get_thresholds():
    """Thresholds are configurable; currently defaults from settings."""
    return settings.RISK_LEVELS


def get_risk_engine():
    md = model_manager.load()
    engine = RiskEngine(model=md, thresholds=_get_thresholds())
    return engine
...
def evaluate_auto_alerts(db: Session, district_id: str | None = None) -> list[Alert]:
    ...
    for pred in latest_risk:
        if pred.risk_score >= 61 and _cooldown_ok(db, "risk", pred.district_id):
            level = pred.risk_level
            sev = "warning" if pred.risk_score < 81 else "critical"
...
def check_rainfall_alert(db: Session, district_id: str | None = None):
    """Create rainfall alert if latest rainfall is extreme."""
    from app.services.i18n import translate

    latest = db.execute(
        select(RainfallRecord).order_by(RainfallRecord.observed_at.desc()).limit(1)
    ).scalars().first()
    if latest and latest.rain_24h >= 200 and _cooldown_ok(db, "rainfall", latest.district_id):
